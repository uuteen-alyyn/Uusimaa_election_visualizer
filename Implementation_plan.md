...
- [ ] Read `submodules/elections/src/{api,data}/` to identify the
      highest-level loaders. Confirm exact function signatures and
      record them in logbook.
- [ ] Build `scripts/build-fixtures.ts`:
  - [ ] Import elections submodule loaders + types (TS path mapping
        in `tsconfig.json` if needed)
  - [ ] For each election in `src/data/catalog.ts`:
    - [ ] Fetch all-vp results
    - [ ] Fetch kunta results, grouped by vp
    - [ ] Fetch top-N candidates per vp + kunta (N configurable, start
          at 40)
    - [ ] Normalize into `RegionResult[]` per
          `src/types/elections.ts`
    - [ ] Write `public/data/elections/{electionId}.json`
  - [ ] For elections with no PxWeb data yet (future), write
        `{ status: "no_data" }` placeholder
  - [ ] Cache PxWeb responses per-run (skip already-fetched within one
        build to avoid re-hitting on retries)
- [ ] Wire `npm run prefetch` → `tsx scripts/build-fixtures.ts`
- [ ] Wire `npm run build` → `prefetch && vite build`
